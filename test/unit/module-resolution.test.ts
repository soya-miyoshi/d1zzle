 ...
 * succeeds, the types are d1zzle's, the editor is happy — and the Worker ships
...
const tsconfig = (target: 'js' | 'dts', withBaseUrl: boolean) => {
	...
	return JSON.stringify({
		compilerOptions: {
			...(withBaseUrl ? { baseUrl: '.' } : {}),
			paths: {
				'drizzle-orm': [`./node_modules/d1zzle/dist/index.${ext}`],
				'drizzle-orm/d1': [`./node_modules/d1zzle/dist/index.${ext}`],
				'drizzle-orm/sqlite-core': [`./node_modules/d1zzle/dist/sqlite-core.${ext}`],
			},
		},
	});
};
...
const bundle = (config: string): { bytes: number; d1zzle: boolean } => {
	...
	// `d1zzle:IsTable` is our own symbol and appears in no other package, which
	...
	return { bytes: out.length, d1zzle: out.includes('d1zzle:IsTable') };
};

beforeAll(() => {
	if (!ready) return;
	workspace = mkdtempSync(join(tmpdir(), 'd1zzle-resolution-'));
	...
	symlinkSync(root, join(modules, 'd1zzle'), 'dir');
...
describe.skipIf(!ready)('the zero-diff migration recipe', () => {
	it('bundles d1zzle', () => {
		expect(bundle(tsconfig('js', true)).d1zzle).toBe(true);
	});
	...
	it('a .d.ts target silently bundles the real drizzle-orm instead', () => {
		...
		expect(fallenThrough.d1zzle).toBe(false);
