...
const WRANGLER_TOML = `name = "rakuteam-api"
main = "src/index.ts"
account_id = "top-level-account"

[[d1_databases]]                       # local (miniflare)
binding = "DB"
database_name = "rakuteam-db-local"
database_id = "local"
...
const WRANGLER_JSONC = `{
	...
	// local (miniflare)
	"d1_databases": [
		{ "binding": "DB", "database_name": "rakuteam-db-local", "database_id": "local" },
	],
	"env": {
		"stg": {
			"account_id": "stg-account",
			"d1_databases": [
				{ "binding": "DB", "database_name": "rakuteam-db-stg", "database_id": "stg-id-1111" },
			],
		},
		"prd": {
			"d1_databases": [
				{ "binding": "DB", "database_name": "rakuteam-db-prd", "database_id": "prd-id-2222" },
			],
		},
	},
}`;
...
describe('parsing environment-scoped wrangler config', () => {
	it('reads [[env.<name>.d1_databases]] out of TOML, alongside the top-level block', async () => {
		...
		expect(config?.env?.['stg']).toEqual({
			account_id: 'stg-account',
			d1_databases: [
				{ binding: 'DB', database_name: 'rakuteam-db-stg', database_id: '__CLOUDFLARE_D1_DATABASE_ID__' },
			],
		});
		expect(config?.env?.['prd']?.d1_databases?.[0]?.database_name).toBe('rakuteam-db-prd');
	});
...
describe('precedence of a resolved value', () => {
	...
	it('lets CLOUDFLARE_D1_DATABASE_ID stand in for a committed placeholder', async () => {
		...
		expect(config.d1.databaseName).toBe('rakuteam-db-stg');
...
describe('saying where it is about to write', () => {
	...
	it('names the environment, binding, database and every source, on remote', async () => {
		...
		expect(log).toContain('rakuteam-db-prd');
	...
	it('reports the local target too, so --local runs are auditable as well', async () => {
		...
		expect(log).toContain('rakuteam-db-local');
